...
const Transaksi = () => {
  ...
  useEffect(() => {
    ...
    const user = JSON.parse(localStorage.getItem("user"))
    setRole(user.role);
    // nyimpen data user dari local storage
  ...
  return (
    <>
      ...
      <div className="w-100 dark:bg-stone-400" >
        ...
        {role === "kasir" && (
          <button className="btn" style={{ backgroundColor: "#FFDEB9", color: "#FC2947" }} onClick={() => {
            modal.show()
          ...
          {!filteredData && (
            transaksi.map((item, index) => (
              <li className="list-group-item m-2" key={`trans${index}-${item.id_transaksi}`}
                style={{ borderTop: `3px solid #800000 `, backgroundColor: "#F8E8EE" }}
              >
                <div className="row" >
                  <div className="col-md-2">
                    <small className="text-danger font-bold text-lg">
                      Tgl.Transaksi
                    </small><br />
                    {item.tgl_transaksi}
                  </div>
                  ...
                  <div className="col-md-2">
                    ...
                    <span className={`badge ${item.status === 'belum_bayar' ? 'bg-danger' : 'bg-#FFB84C'}`} style={{ backgroundColor: `#FFB84C`, fontSize: `14px ` }} >
                      {item.status}
                    </span> <br />
                ...
                <div className="row">
                  <b><h5 className="ms-1 p-2" >  Detail Pesanan</h5></b>
                  <ul className="list-group" style={{ backgroundColor: "#FDF4F5" }} >
                    {item.detail_transaksi.map((detail) => (
                      <li className="list-group-item" key={`detail${item.id_transaksi}`}>
                        <div className="row">
                          {/**nama pesanan */}
                          <div className="col-md-3">
                            <small className="text-danger">Menu</small> <br />
                            {detail.menu.nama_menu}
                          </div>
                          {/**jumlah pesanan */}
                          <div className="col-md-3">
                            <small className="text-danger">Jumlah</small><br />
                            qty: {detail.jumlah}
                          </div>
                          <div className="col-md-3">
                            <small className="text-danger">Harga</small><br />
                            @ {detail.harga}
                          </div>
                          <div className="col-md-3">
                            <small className="text-danger">Total Harga</small><br />
              ...
              >
                <div className="row" >
                  ...
                  <div className="col-md-2">
                    ...
                    <span className={`badge ${item.status === 'belum_bayar' ? 'bg-danger' : '-#6096B4'}`} style={{ backgroundColor: `#FFB84C`, fontSize: `14px ` }} >
                      {item.status}
                    </span> <br />
                    {item.status === 'belum_bayar' ?
                      <>
                ...
                {/* *list menu yang dipesan // style={{ backgroundColor: "#F8E8EE", width: 'auto' }} */}
                <div className="row" >
                  <b><h5 className="ms-1 p-2" >Detail Pesanan</h5></b>
                  <ul className="list-group" >
                    {item.detail_transaksi.map((detail) => (
